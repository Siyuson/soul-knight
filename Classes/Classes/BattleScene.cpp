...
bool BattleScene::init() {
    ...
    // add knight to scene
    this->knight = Knight::KnightCreate("Character//Knight.png");

    this->knight->sprite->setPosition(
        Point(visibleSize.width / 2, visibleSize.height / 2));

    this->knight->sprite->setScaleX(0.3f);
    this->knight->sprite->setScaleY(0.3f);


    this->addChild(this->knight->sprite, 4);
    //add knight to scene
